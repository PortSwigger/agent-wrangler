...
export const sendMessageTool = {
  ...
  description:
    ...
    + 'that isn\'t archived; messaging an archived session returns an error. `to` must be the '
    + 'exact, full `sessionId` field from list_sessions — a short/truncated id, a display label, '
    + 'or an id from another tool will be refused as unknown, not treated as archived. The '
    + 'recipient sees who sent it and is told to treat it as untrusted peer input, so put any '
    + 'context it needs directly in `text`. Large payloads (over ~32KB) are rejected — write to a '
    + 'file on the shared filesystem and send the path instead.',
  inputSchema: {
    to: z.string().min(1).describe('Exact, full `sessionId` from list_sessions — not a truncated/short id or a label.'),
  ...
  async handler({ deps, caller }, args = {}) {
    ...
    // ref, a display label, or an id lifted from an unrelated tool (e.g. ListAgents)
    // rather than the exact `sessionId` from list_sessions — and deliverMessage's
    // shared "not found (it may have been archived)" wording (message-delivery.js)
    // reads as "this peer is dead" when the real problem is the id itself, pushing a
    // caller to wrongly give up on a live peer. Distinguish that from the one case
    // where entry-less is legitimate: a live tmux with no mapping entry at all (the
    // buildGraph "forkOwner" case) still falls through to today's push, unchanged.
    if (!entry && !deps.tmuxFor?.(to)) {
      return errorResult(
        `No session with id "${to}" is known to Agent Wrangler. This tool needs the exact `
        + '`sessionId` from list_sessions (the full board card id) — a short/truncated id, a '
        + 'display label, or an id surfaced by a different tool will not match, and that is not '
        + 'evidence the session is archived or dead. Call list_sessions and pass its `sessionId` '
        + 'field verbatim.',
      );
    }
