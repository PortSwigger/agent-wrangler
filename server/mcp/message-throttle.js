...
//
// Two layers, both keyed on the UNORDERED pair {from,to} so an A↔B ping-pong is
// measured as one conversation regardless of direction:
//   - cooldown: a minimum gap since the pair's last delivered message — stops
//     rapid-fire bursts.
//   - rate limit: at most maxPerWindow delivered messages per rolling windowMs —
//     catches a loop that paces itself just above the cooldown (e.g. one every 5s
  ...
  function check(from, to) {
    ...
    if (recent.length) {
      ...
      if (sinceLast < cooldownMs) {
        ...
        return {
          ok: false,
          error: `Rate limited: you messaged this session moments ago — wait ~${wait}s before `
            + 'sending again. If you were only acknowledging, do not reply at all.',
        };
      }
    }
    if (recent.length >= maxPerWindow) {
      return {
        ...
        error: `Rate limited: too many messages with this session recently (max ${maxPerWindow} `
          ...
          + 'you have substantive new information they need.',
      };
    }
    return {
      ok: true,
