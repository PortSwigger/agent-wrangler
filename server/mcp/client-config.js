...
// touch the memo or the shared config.json — same seam as taskMemoryEnabled's
// callers.
export function allowedToolsArg({ ext = getExtensions() } = {}) {
  return [...ALLOWED_TOOLS, ...ext.allowedToolNames].map(allowedToolName).join(',');
}
