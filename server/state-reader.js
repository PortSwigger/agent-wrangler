...
export async function buildGraph(sessionManager, enrich, { runtimeResolver = runtimeFor, discover = discoverClaudeSessions, capture = capturePane, mailStore } = {}) {
  ...
  for (const s of sessions) {
    ...
    const session = {
      ...
      autoRebaseLinkedPr: mapEntry?.autoRebaseLinkedPr ?? false,
  ...
  for (const d of primary) {
    ...
    const session = {
      ...
      autoRebaseLinkedPr: appEntry?.autoRebaseLinkedPr ?? false,
  ...
  for (const entry of sessionManager?.activeEntries?.() || []) {
    ...
    const session = {
      ...
      autoRebaseLinkedPr: entry.autoRebaseLinkedPr ?? false,
