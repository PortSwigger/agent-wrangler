...
// The full single-session resume flow — shared by the WS `resume` handler and
// the bulk task-restore path (taskUnarchiveHandler resuming every session a
// task-archive cascaded), so relaunch, memory-binding, and the
// archived-task-fallback logic below can't drift between the two callers.
export async function resumeSession(sessionId, ctx, { recreateDir, killJobsFirst } = {}) {
  const s = ctx.sessionFromGraph(sessionId);
  ...
  // under that task's tile if the task were restored later, with nothing
  ...
  const socket = ctx.sessionManager.entryFor(sessionId)?.socket ?? '';
...
export const resumeHandler = {
  ...
  async handler(msg, ctx) {
    await resumeSession(msg.sessionId, ctx, { recreateDir: msg.recreateDir, killJobsFirst: msg.killJobsFirst });
  },
};
