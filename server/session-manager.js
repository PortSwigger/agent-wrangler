...
export class SessionManager {
  ...
  prLinks() {
    ...
    for (const [sessionId, entry] of this.map)
      for (const l of entry.links || [])
        if (l.type === 'pr' && l.url)
          out.push({ ownerId: sessionId, url: l.url, number: l.number, checkStatus: l.checkStatus, dirty: l.dirty, unresolvedCount: l.unresolvedCount, rebaseFailureKey: l.rebaseFailureKey, rebaseBlockedKey: l.rebaseBlockedKey });
    return out;
  }
  ...
  async dispatch({ cwd, intent = '', model, effort, autoCompactTokens, agent = 'claude', runtime = 'local', addDirs = [], bindMemory,
                   ...
                   autoRebaseLinkedPr, autoMergeOnPass, workflow: workflowOpt, spawnedBy, parentSession } = {}) {
