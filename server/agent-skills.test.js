...
test('the real agent-skills dir ships task-memory, links, mail, checklist, job-worker, spawn-session, session-activity, session-hierarchy, adversarial-pr-review, and advisor with descriptions', () => {
  const names = skillEntries().map((e) => e.name);
  assert.deepEqual(names, ['adversarial-pr-review', 'advisor', 'checklist', 'job-worker', 'links', 'mail', 'session-activity', 'session-hierarchy', 'spawn-session', 'task-memory']);
