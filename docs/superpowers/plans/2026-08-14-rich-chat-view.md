...
- **Initial window.** `sinceOffset == null` means "first open": read only the trailing `WINDOW_BYTES` of the file and set `more: true` if bytes were skipped. `sinceOffset` bounds the *tail* only, so without this a months-old transcript is parsed and shipped whole.
...
**Two things intentionally deferred within Task 5.** The spec's "load earlier" affordance has its server side built (`more: true` plus a backwards read from the window start) but no client control yet — Task 9 renders the stream without it. Wire the button when the first long transcript makes it necessary; the protocol already supports it. Flagged rather than silently dropped.
