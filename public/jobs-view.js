...
import { JOB_COLUMNS, jobCards, jobCardHtml, jobNeedsReview, jobStatus, receiptHtml, dependencyLevels, cancelledDependencies } from './jobs.js';
...
export function initJobsView({ send, getAgents, onSession, onDiff, onBoard }) {
  ...
  function render() {
    ...
    q('#jobs-filter').innerHTML = `<option value="">All jobs</option>${data.jobs.map((j) => `<option value="${esc(j.id)}" ${filter === j.id ? 'selected' : ''}>${esc(j.title)}</option>`).join('')}`;
    const visible = cards.filter((c) => (!filter || c.job.id === filter) && (!needsOnly || jobNeedsReview(c.job, c.sub)) && (showDone || c.sub?.stage !== 'done'));
    q('#jobs-board').innerHTML = JOB_COLUMNS.map(([stage, title, note], i) => {
      const column = visible.filter((c) => c.stage === stage);
      return `<section class="job-column" aria-label="${esc(title)}"><header><span class="job-column-number">0${i + 1}</span><h2>${esc(title)}</h2><span class="job-column-count">${column.length}</span></header><p>${esc(note)}</p><div class="job-column-cards">${column.map(jobCardHtml).join('') || `<div class="job-empty">${stage === 'backlog' && !data.jobs.length ? 'Start with an outcome.<br>Wrangler will shape the work.' : 'Clear for the next step'}</div>`}</div></section>`;
    }).join('');
  ...
  function renderDetail() {
    ...
    if (planDraft) {
      ...
      body = `<h3>Business value</h3><div class="job-stories">${planDraft.stories.map((s) => `<div><b>${esc(s.key)} · ${esc(s.title)}</b><p>${esc(s.value)}</p></div>`).join('')}</div>
        <h3>Landing order <small>Same wave can land independently</small></h3><div class="job-plan-table"><table><thead><tr><th>Wave</th><th>Proposed PR title</th><th>Repository / Jira story</th><th>Deploy after</th></tr></thead><tbody>${planDraft.subJobs.map((s, i) => `<tr><td>${levels.get(s.id) + 1}</td><td><input aria-label="PR title ${i + 1}" data-title="${i}" maxlength="180" value="${esc(s.title)}"></td><td><span class="job-plan-repo" title="${esc(s.repo)}">${esc(tildify(s.repo))}</span><span class="job-plan-story">${esc(planDraft.stories.find((t) => t.id === s.storyId)?.key)}${s.jiraKey ? ` / ${esc(s.jiraKey)}` : ''}</span></td><td><details><summary>${s.dependsOn.length ? s.dependsOn.map((id) => esc(planDraft.subJobs.find((d) => d.id === id)?.title || id)).join(', ') : 'Independent'}</summary>${planDraft.subJobs.filter((d) => d.id !== s.id).map((d) => `<label><input type="checkbox" data-dep="${i}" value="${esc(d.id)}" ${s.dependsOn.includes(d.id) ? 'checked' : ''}>${esc(d.title)}</label>`).join('') || 'No dependencies'}</details></td></tr>`).join('')}</tbody></table></div>
        <details class="job-more"><summary>Deployment checks & implementation detail</summary>${planDraft.subJobs.map((s) => `<h4>${esc(s.title)}</h4><p>${esc(s.instructions)}</p><p>Deployment: ${esc(s.deployment.workflows.join(', '))}</p><p>${esc(s.deployment.verify)}</p>`).join('')}</details>
        <p class="job-authority">Approve starts local work in dedicated worktrees. ${job.reviewCode ? 'Code review is on.' : 'Verified work will publish automatically.'} ${job.reviewMerge ? 'You approve each merge.' : 'Green PRs merge automatically.'}</p>
      ...
      body = `<p class="job-detail-meta">${esc(sub.jiraKey)} · ${esc(tildify(sub.repo))}</p>${deps.length ? `<div class="job-dependency-list">${deps.map((d) => `<span>${d?.deployed ? '✓' : d?.cancelledAt ? '×' : '↳'} Deploy after ${esc(d?.title)}${d?.cancelledAt ? ' (cancelled)' : ''}</span>`).join('')}</div>` : ''}
        ...
        <div class="job-actions">${sub.stage === 'implementation' && sub.local && job.reviewCode && !sub.codeApprovedAt && sub.dependenciesVerified ? '<button class="primary" data-action="approve-code">Approve code</button>' : ''}${sub.stage === 'pr' && sub.pr?.checkStatus === 'passing' && job.reviewMerge && sub.mergeApprovedHead !== sub.pr.head ? '<button class="primary" data-action="approve-merge">Approve merge</button>' : ''}${sub.stage === 'implementation' && sub.local ? '<button id="job-revise">Request code changes</button>' : ''}${sub.worktree && sub.stage !== 'done' ? '<button id="job-diff">Review code in Wrangler</button>' : ''}${sub.sessions.length ? `<button id="job-session">${onBoard(sub.sessions.at(-1)) ? 'Open session' : 'Restore session'}</button>` : ''}</div>
    ...
    dialog.querySelectorAll('[data-action]').forEach((b) => b.onclick = () => {
      ...
      action(name, name === 'approve-plan' ? { plan: planDraft, revision }
        : name === 'retry' ? { subJobId: sub?.error ? sub.id : undefined }
          : { head: sub?.pr?.head, localReceiptId: sub?.local?.receiptId });
    });
    ...
    bind('#job-cancel', () => {
      ...
      show(`<h2>Cancel ${esc(sub.title)}?</h2><p>This skips straight to cleanup. ${live ? 'The running step is stopped and its receipt is ignored. ' : ''}${sub.pr ? 'The pull request stays open on GitHub for you to close. ' : ''}Sessions are archived; the worktree is removed only if its commits are already pushed or the branch is unchanged. Sub-jobs that deploy after this one will need cancelling too.</p><div class="job-actions"><button class="danger" data-action="cancel">Cancel sub-job</button><button id="job-cancel-back">Keep working</button></div>`);
  ...
  function createJob() {
    ...
    form.onsubmit = (e) => {
      ...
      send({ type: 'job-create', job: { title: values.get('title'), intent: values.get('intent'), repos: [...new Set(values.get('repos').split('\n').map((s) => s.trim()).filter(Boolean))], agent: values.get('agent'), model: values.get('model') || '', planningPrompt: values.get('planningPrompt'), reviewCode: values.has('reviewCode'), reviewMerge: values.has('reviewMerge'), updateMain: values.has('updateMain') } });
    };
  }
  ...
  q('#jobs-board').onclick = (e) => { const card = e.target.closest('[data-job]'); if (card) openDetail(card.dataset.job, card.dataset.sub); };
