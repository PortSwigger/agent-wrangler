...
import { JOB_COLUMNS, jobCards, jobCardHtml, jobBoardHeaderHtml, jobNeedsReview, jobStatus, receiptHtml, dependencyLevels, cancelledDependencies, commentVerdict, redComments, mergeHeldByComments, COMMENT_TONE_LABEL } from './jobs.js';
...
export function initJobsView({ send, getAgents, onSession, onDiff, onBoard }) {
  ...
  function boardHtml(job, cards) {
    const columns = JOB_COLUMNS.map(([stage, title, note], i) => {
    ...
    return `<section class="job-board" data-board="${esc(job.id)}" aria-label="${esc(job.title)}">${jobBoardHeaderHtml(job)}<div class="job-board-columns">${columns}</div></section>`;
  }
  function render() {
